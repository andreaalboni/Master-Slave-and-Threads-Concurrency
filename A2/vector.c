...
void download(monitor_t *mon, int k, vector_t *V) 
{
    ...
    while (mon->next_size == 0 || mon->next_size > k)
        pthread_cond_wait(&mon->can_consume, &mon->mutex);
    

    from_buffer(mon, V);
    pthread_cond_signal(&mon->can_produce);
...
void monitor_init(monitor_t *mon)
{
    ...
    pthread_cond_init(&mon->can_produce, NULL);
    pthread_cond_init(&mon->can_consume, NULL);
...
void monitor_destroy(monitor_t *mon) 
{
    ...
    pthread_cond_destroy(&mon->can_produce);
    pthread_cond_destroy(&mon->can_consume);
}
